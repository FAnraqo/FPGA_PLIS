  ...
  while (mode != 3) {
    switch (mode) {
      ...
      case 2: {
        ...
        char byte;
        while (inputFile.get(byte)) { // Читаем по одному байту из исходного файла
          int bitToFlip = rand() % 7 + 1; // Генерируем случайное число от 1 до 7
          byte ^= (1 << (bitToFlip - 1)); // Инвертируем выбранный бит
          outputFile.put(byte); // Записываем измененный байт в выходной файл
        }

        inputFile.clear(); // Сбрасываем флаги ошибок, включая eof

        // Перемещение указателя чтения на начало файла
        ...
        while (inputFile.get(byte)) { // Читаем по одному байту из исходного файла
          int countFlipBit = rand() % 7;
          for (int i = 0; i < countFlipBit; i++){
            int bitToFlip = rand() % 7 + 1; // Генерируем случайное число от 1 до 7
            byte ^= (1 << (bitToFlip - 1)); // Инвертируем выбранный бит
          }
          outputFile.put(byte); // Записываем измененный байт в выходной файл
        ...
        // Выводим битовую последовательность файла с ошибками
